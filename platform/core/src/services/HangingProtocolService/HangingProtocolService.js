...
class HangingProtocolService {
  ...
  _updateViewports() {
    ...
    // based on some rules
    const matchedUids = {};
    this.customAttributeRetrievalCallbacks.alreadyMatched = (metadata) => {
      return !!matchedUids[metadata.uid];
    }
    ...
    stageModel.displaySets.forEach(displaySet => {
      ...
      if (bestMatch) {
        const { displaySetInstanceUID } = bestMatch;
        matchedUids[displaySetInstanceUID] = true;
